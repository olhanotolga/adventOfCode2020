let passportsInput = `iyr:1928 cid:150 pid:476113241 eyr:2039 hcl:a5ac0f
ecl:#25f8d2
byr:2027 hgt:190
...
let sampleInput = `pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
function detectValidPassports(input) {
	const passes = input.split(`\n\n`).map(pass => pass.split(/\s/).reduce((acc, curr) => {
			acc[curr.substring(0,3)] = curr.substring(4);
			return acc
		}, {})
	);
	console.log(passes.length);
	let validPasses = passes.filter(pass => {
		return (
			(Number(pass['byr']) >= 1920 && Number(pass['byr']) <= 2002) && 
			(Number(pass['iyr']) >= 2010 && Number(pass['iyr']) <= 2020) && 
			(Number(pass['eyr']) >= 2020 && Number(pass['eyr']) <= 2030) && 
			(((parseInt(pass['hgt']) >= 150 && parseInt(pass['hgt']) <= 193) && pass['hgt'] === (parseInt(pass['hgt']) + 'cm')) || ((parseInt(pass['hgt']) >= 59 && parseInt(pass['hgt']) <= 76) && (pass['hgt'] === (parseInt(pass['hgt']) + 'in')))) && 
			(pass['hcl'] && pass['hcl'].match(/^#[0-9a-f]{6}$/)) && 
			(pass['ecl'] && pass['ecl'].match(/^(amb|blu|brn|gry|grn|hzl|oth)$/)) && 
			(pass['pid'] && pass['pid'].match(/^\d{9}$/))
			)
	});

	// const check = validPasses.filter(pass => Object.entries(pass).length < 8 && pass['cid'])
	// .slice(100)
	// console.log(check, check.length);
	console.log(validPasses.length);
}

detectValidPassports(passportsInput);
// detectValidPassports(sampleInput);