let pwdsAndPolicies = `2-8 t: pncmjxlvckfbtrjh
8-9 l: lzllllldsl
...
function findValidPasswords(input) {
	let inputArr = input.split('\n');
	let validPwds = 0;
	
	for (let line of inputArr) {
		let firstSpace = line.indexOf(' ')
		// store count from and count to as array elements
		let fromTo = line.substring(0, firstSpace).split('-');
		// make character a pattern
		let char = new RegExp(line[firstSpace+1], 'g')
		// find all matches of a character (-1 in the policy)
		let characterMatches = line.match(char).length - 1;
		
		if (characterMatches >= Number(fromTo[0]) && characterMatches <= Number(fromTo[1])) {
			validPwds++;
		}
...
console.log(findValidPasswords(pwdsAndPolicies));

function findValidPasswords2(input) {
	let inputArr = input.split('\n');
	let validPwds = 0;
	...
	for (let line of inputArr) {
		let firstSpace = line.indexOf(' ')
		// store count from and count to as array elements
		let oneTwo = line.substring(0, firstSpace).split('-');
		let posOne = Number(oneTwo[0]) - 1;
		let posTwo = Number(oneTwo[1]) - 1;
		// single out the character
		let char = line[firstSpace+1];

		// single out the substring:
		let potentialPwd = line.substring(line.lastIndexOf(' ') + 1);

		// console.log(char);
		if (potentialPwd[posOne] === char && potentialPwd[posTwo] === char) {
			continue;
		}
		if (potentialPwd[posOne] === char || potentialPwd[posTwo] === char) {
			validPwds++;
...
console.log(findValidPasswords2(pwdsAndPolicies));
