...
function countUniqueAnswers(input) {
	const answersByGroups = input.split('\n\n').map(answ => answ.replace(/\n/g, '').split(''));
	// console.log(answersByGroups);
	const uniqueAnswers = answersByGroups.map(ans => new Set(ans).size);
	// console.log(uniqueAnswers);
	const sumOfUniqueAnswers = uniqueAnswers.reduce((acc, curr) => acc + curr, 0);
	console.log(sumOfUniqueAnswers);
	return sumOfUniqueAnswers;
}

countUniqueAnswers(questions);
...
function countUniversalAnswers(input) {
	const answersByGroups = input.split('\n\n');
	console.log(answersByGroups);

	let universalAnswers = 0;

	for (let group of answersByGroups) {
		let numOfMembers = 1;
		if (/\n/.test(group)) {
			numOfMembers += group.match(/\n/g).length;
		}
		console.log(numOfMembers);

		if (numOfMembers === 1) {
			universalAnswers += group.length;
		} else {
			let lettersMap = {}
			for (letter of group) {
				if (letter !== '\n') {
			...
			for (let letter in lettersMap) {
				if (lettersMap[letter] === numOfMembers) {
					universalAnswers++;
				}
			}
		}
	...
	console.log('universal answers:', universalAnswers);
}

countUniversalAnswers(questions)