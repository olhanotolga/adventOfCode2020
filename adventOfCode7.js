let bagRules = `shiny purple bags contain 2 pale blue bags, 1 wavy fuchsia bag, 5 pale salmon bags.
...
function findSuitableBags(input, currentBagsPattern) {
	// all bags containing shiny gold ones:
	// let currentBagsPattern = /^([a-z]+ [a-z]+ bags?) contain.+(shiny gold bags?).*\.$/gm;
	// let currentBagsPattern = '(striped coral bags?)';
	let searchPattern = new RegExp('^([a-z]+ [a-z]+ bags?) contain.+' + currentBagsPattern + '.*\\.$', 'gm');
	
	// console.log(searchPattern);

	if (input.match(searchPattern)) {
		let matches = input.match(searchPattern);
		
		// console.log(matches);

		for (let rule of matches) {
			let newBag = rule.match(/^[a-z]+ [a-z]+ bags?/)[0];
			let newBagPattern = '(' + newBag + '?)';
			// console.log(newBag);
			totalBags.push(newBag);
			findSuitableBags(input, newBagPattern);
		}
	}

}
let currentBag = '(shiny gold bags?)';
let totalBags = [];
findSuitableBags(bagRules, currentBag, totalBags);
// console.log(totalBags, totalBags);

let uniqueBags = new Set(totalBags);
console.log('unique bags:', uniqueBags.size);

let sampleRules = `shiny gold bags contain 2 dark red bags.
...
let arrayInput = sampleRules.split('\n');

function countBagsWithin(input, currentBag, qty) {

	let currentBagPattern = '^(' + currentBag + ')';
	console.log(currentBagPattern);

	let currentBagLinePattern = new RegExp(currentBagPattern + 's contain.+\.$', 'gm');
	let currentBagLine = input.match(currentBagLinePattern)[0];
	console.log(currentBagLine);
	
	let innerBagsPattern = /\d+ [a-z]+ [a-z]+ bag/g;

	let matches = currentBagLine.match(innerBagsPattern);
	console.log(matches);

	if (matches) {
		for (let match of matches) {
			console.log(match);
			let firstSpaceIndex = match.indexOf(' ');
			// let lastSpaceIndex = match.lastIndexOf(' ');
			let numOfInnerBags = Number(match.substring(0, firstSpaceIndex));
			bags += qty * numOfInnerBags;
			console.log(bags)
			// qty * numOfInnerBags
			newCurrentBag = match.substring(firstSpaceIndex + 1)
			
			countBagsWithin(input, newCurrentBag, qty * numOfInnerBags)
		}
	}
}

let targetBag = 'shiny gold bag';
let bags = 0;
countBagsWithin(bagRules, targetBag, 1);
console.log(bags);