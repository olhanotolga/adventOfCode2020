...
let real = `LLLLLLL.L.LL.L.LLLL.LLLLLLL.LLLLL.LLLLLL.LLLLLLL.L.LLL.LLLLLLLL.LLLLLL.LLLLLLLLLLLLLL.LLLLL
...
function checkAndMarkSeat(y, x, ferry) {
	// marks the seat as empty or occupied based on occupied adjacent seats
	let seat = ferry[y][x]
	// console.log(seat);
	
	if (seat === '.') return seat
	
	// counts the the num of occupied adjacent seats
	let occupiedSeats = 0;
	
	if (ferry[y-1]) {
		if (ferry[y-1][x-1] === '#') occupiedSeats++
		if (ferry[y-1][x] === '#') occupiedSeats++
		if (ferry[y-1][x+1] === '#') occupiedSeats++
	}
	
	if (ferry[y][x-1] === '#') occupiedSeats++
	if (ferry[y][x+1] === '#') occupiedSeats++
	...
	if (seat === 'L' && occupiedSeats === 0) seat = '#'
	if (seat === '#' && occupiedSeats >= 4) seat = 'L'

	// console.log(seat);
	return seat;
}

function seatIterator(area, checkerFunction) {
	let rows = area.length
	let seats = area[0].length;
	...
	for (let r = 0; r < rows; r++) {
		...
		for (let s = 0; s < seats; s++) {
			let newSeat = checkerFunction(r, s, area)
			if (newSeat === '#') occupiedSeats++
			newRow += newSeat
		}
		newArea.push(newRow)
	}

	// console.log({newArea, occupiedSeats});
	return {newArea, occupiedSeats}
}

function runSeatIterator(inputArea, checkSeatFunc) {
	let prevOccupiedSeats = 0;
	let currentArea = seatIterator(inputArea, checkSeatFunc).newArea
	let currOccupiedSeats = seatIterator(inputArea, checkSeatFunc).occupiedSeats
	
	while (currOccupiedSeats !== prevOccupiedSeats) {
		prevOccupiedSeats = currOccupiedSeats
		currentArea = seatIterator(currentArea, checkSeatFunc).newArea
		currOccupiedSeats = seatIterator(currentArea, checkSeatFunc).occupiedSeats
	}
	console.log({currentArea, currOccupiedSeats});
	return {currentArea, currOccupiedSeats}
}

// runSeatIterator(realArray, checkAndMarkSeat) // 2166
...
//? If a seat is occupied (#) and five or more visible seats are also occupied, the seat becomes empty.
//? Otherwise, the seat's state does not change.

function checkAndMarkSeat2(y, x, ferry) {
	// marks the seat as empty or occupied based on visible occupied seats
	let seat = ferry[y][x]
	// console.log(seat);
	
	if (seat === '.') return seat
	
	// counts the the num of visible occupied seats
	let occupiedSeats = 0;
	
	// while there are seats/rows to iterate && while the seat status is visible
	
	if (ferry[y - 1]) {
		if (ferry[y-1][x-1] === '#') {
			occupiedSeats++
		} else if (ferry[y-1][x-1] === '.') {
			let a = 2
			while(ferry[y-a] && ferry[y-a][x-a] === '.') {
				a++
			}
			if (ferry[y-a] && ferry[y-a][x-a] === '#') {
				occupiedSeats++
			}
		}
		if (ferry[y-1][x] === '#') {
			...
			let a = 2
			while(ferry[y-a] && ferry[y-a][x] === '.') {
				a++
			}
			if (ferry[y-a] && ferry[y-a][x] === '#') {
				occupiedSeats++
			}
		}
	...
	if (seat === 'L' && occupiedSeats === 0) seat = '#'
	if (seat === '#' && occupiedSeats >= 5) seat = 'L'
...
runSeatIterator(realArray, checkAndMarkSeat2); // 1955